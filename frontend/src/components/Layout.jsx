      ...
      <aside className={"fixed lg:static inset-y-0 left-0 z-40 w-64 bg-slate-900 text-white transform transition-transform duration-300 flex flex-col " + (open ? "translate-x-0" : "-translate-x-full lg:translate-x-0")}>
        <div className="p-6 border-b border-slate-700">
          <Link to="/dashboard" className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-bold text-lg leading-tight">ISE StudyTracker</h1>
              <p className="text-xs text-slate-400">Uni Duisburg-Essen</p>
            </div>
          </Link>
        </div>
        ...
        <div className="p-4 border-t border-slate-700">
          <div className="flex items-center gap-3 mb-3 px-2">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-slate-400">Semester {user ? user.semester : 1}</p>
            </div>
          </div>
          <button onClick={() => { logout(); go("/"); }}
            className="w-full text-left px-4 py-2 text-sm text-slate-300 hover:text-red-400 hover:bg-slate-800 rounded-lg">Logout</button>
        </div>
      </aside>
      ...
      <div className="flex-1 flex flex-col min-h-screen min-w-0">
        <header className="bg-white border-b border-gray-200 px-4 lg:px-8 py-4 flex items-center gap-4 sticky top-0 z-20">
          ...
          <h2 className="text-lg font-semibold text-gray-800 flex-1">
            {(nav.find((n) => n.path === loc.pathname) || {}).label || "ISE StudyTracker"}
          </h2>
        </header>
